...
def process_images(image_folder, intri_folder, c2w_folder, output_folder, anchor_image, anchor_path, depth_path, mask_folder = None):
    # Load the anchor image intrinsics and c2w
    K_anchor = np.load(os.path.join(intri_folder, f'{anchor_image}.npy'))
    c2w_anchor = np.load(os.path.join(c2w_folder, f'{anchor_image}.npy'))
    ...
    cv2.imwrite(depth_path, depth_anchor)
    ...
    for image_name in sorted(os.listdir(image_folder)):
        if image_name.endswith('.png'):  # Ensure it's an image file
            ...
            intri_path = os.path.join(intri_folder, image_name.replace('.png', '.npy'))
            c2w_path = os.path.join(c2w_folder, image_name.replace('.png', '.npy'))
...
# Example usage
process_images(
    image_folder='/workspace/data/spinnerf-dataset/4/images_4',
    intri_folder='/workspace/knguyen/gaussian-splatting/output/4/train/ours_30000/intri',
    c2w_folder='/workspace/knguyen/gaussian-splatting/output/4/train/ours_30000/c2w',
    output_folder='output/not_mask',
    anchor_image='20220819_105734',
    anchor_path='/workspace/knguyen/sdxl/output.png',
    depth_path='/workspace/knguyen/sdxl/controlnet_output.png',
    mask_folder='/workspace/data/spinnerf-dataset/4/seg',
)
